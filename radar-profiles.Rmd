...
```{r}
generate_plot_vertical_distribution <- function(alts, radar, season, day_night) {
  alts %>%
    filter(season == !!season, day_night == !!day_night) -> a

  cur_radar <- a %>% filter(radar == !!radar)
  ...
  ggplot() +
    geom_col(aes(x = height, y = dens_prop, fill = factor(height_class, levels = c(
      "0-200 m (EI height)", "201-400 m",
      "401-600 m", "601-800 m", "801-1000 m", "1000+ m"
    ))), data = a %>% filter(radar == !!radar), just = 0) +
    geom_vline(xintercept = 200, color = "white", linewidth = 1) +
    geom_vline(xintercept = 1000, color = "white", linewidth = 1) +
    scale_fill_manual(values = c("#CC0001", "#1B4675", "#003A6D", "#07264E", "#061735", "#060a19"), name = "Vlieghoogte", guide = "none") +
    scale_x_continuous(breaks = c(0, 200, 400, 600, 800, 1000, 1050), labels = c("0", "200", "400", "600", "800", "1000", "1000+")) +
    scale_y_continuous(limits = y_lim) +
    scale_color_manual(values = c("gray70", "white"), guide = "none") +
    coord_flip(expand = FALSE, xlim = c(0, 1050)) +
    theme_minimal(base_size = 16, base_family = "Helvetica") +
    labs(x = "Flight altitude (m agl)", y = "% of migration", title = paste0("Hoogteprofiel ", radar, " radar"), subtitle = paste0(dag_nacht, " in het ", seizoen))
}

plot_flight_altitudes <- generate_plot_vertical_distribution(altitudes, "Herwijnen", "autumn", "night")
plot_flight_altitudes
```
...
generate_plot_vertical_distribution_pie <- function(alts, radar, season, day_night) {
  alts_pie_classes <- alts %>%
    filter(radar == !!radar, season == !!season, day_night == !!day_night) %>%
    group_by(height_class, radar) %>%
  ...
  alts_observations$height_class <- forcats::fct_rev(alts_observations$height_class)
  
  ggpie::ggdonut(group_key = "height_class", count_type = "full", 
                 data = alts_observations,
                 label_info = "ratio", label_pos = "out", label_color = "black", label_size = 4, label_type = "circle", label_gap = 0.25,
    ...
    scale_fill_manual(values = rev(c("#CC0001", "#1B4675", "#003A6D", "#07264E", "#061735", "#060a19")), name = "Vlieghoogte",
                      guide = guide_legend()) +
    theme_minimal(base_size = 16, base_family = "Helvetica") +
    annotate("text", x = 0, y = 0, label = paste0(radar), color = "black", size = 7) +
...
plot_flight_altitudes_classes <- generate_plot_vertical_distribution_pie(altitudes, "Herwijnen", "autumn", "night")
plot_flight_altitudes_classes
```
