...
save_map <- function(x) {
  ...
  # @TODO: I can imagine there is a difference between calculating the centerpoint of a raster
  # based on the raster's extent vs. calculating it based on the radar's location. Might fix
  # in the future, but cannot be pressing.
  d1 <- init(r1, fun = NA_real_)
  d2 <- init(r2, fun = NA_real_)
  
  center_d1_x <- mean(c(ext(d1)$xmin, ext(d1)$xmax))
  center_d1_y <- mean(c(ext(d1)$ymin, ext(d1)$ymax))
  
  center_d2_x <- mean(c(ext(d2)$xmin, ext(d2)$xmax))
  center_d2_y <- mean(c(ext(d2)$ymin, ext(d2)$ymax))
  
  center_d1 <- cellFromXY(d1, cbind(center_d1_x, center_d1_y))
  center_d2 <- cellFromXY(d2, cbind(center_d2_x, center_d2_y))
  
  d1[center_d1] <- 0
  d2[center_d2] <- 0
  
  p <- 2
  d1_dist <- 1/((distance(d1) + 0.00000001)^p)
  d2_dist <- 1/((distance(d2) + 0.00000001)^p)
  
  d1_resampled <- resample(d1_dist, rn)
  d2_resampled <- resample(d2_dist, rn)
  
  rw1 <- r1_resampled * d1_resampled
  rw2 <- r2_resampled * d2_resampled
  
  m <- mosaic(sprc(rw1, rw2), fun = sum) / 
    mosaic(sprc(d1_resampled, d2_resampled), fun = sum)
  ...
  ggplot(data) +
    geom_tile(aes(x = x, y = y, fill = vir_mean)) +
