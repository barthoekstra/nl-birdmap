...
```{r}
stack_rbcs <- function(rbcs, props, year_season, odimcode, daytime = "night", threshold_max = 2500, mean_method = "scans", R_threshold = NULL, overwrite = FALSE) {
...
generate_stacks <- function(year_season, day_night, odim_code,
  ...
  lapply(gr, function(x) {stack_rbcs(rbcs = rbcs, props = files$prop, year_season = year_season, odimcode = odim_code, 
                                     threshold_max = as.numeric(x["th"]), R_threshold = as.numeric(x["R_threshold"]), overwrite = overwrite)})
}

processing_stacking <- pbmcmapply(generate_stacks, year_season = g$year_season, day_night = g$day_night, odim_code = g$odim_code,
