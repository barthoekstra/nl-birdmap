...
bind_cols(trb_wpr_nhol, trb_ban_nhol, trb_bsn_nhol) %>%
  dplyr::select(power_density, MT_autumn, MT_spring) %>%
  pivot_longer(cols = c(MT_autumn, MT_spring), names_to = "season", values_to = "MT") %>%
  mutate(season = case_when(season == "MT_autumn" ~ "Autumn",
                            season == "MT_spring" ~ "Spring"),
         type = "Existing turbines") %>%
  identity() -> wpr_trb_nhol

bind_rows(wpr_trb_nhol, wpr_bkg_nhol, wpr_res_nhol) %>%
  mutate(season = fct_relevel(season, c("Spring", "Autumn")),
         MT = MT / 1000) %>%
  identity() -> wpr_data
...
ggplot() +
  geom_bin_2d(aes(x = MT, y = power_density, fill = after_stat(ndensity)),
              data = wpr_data %>% filter(type == "Background") %>% dplyr::select(MT, power_density, season), bins = 25) +
  geom_hdr_lines(aes(x = MT, y = power_density, color = type),
                 data = wpr_data %>% filter(type %in% c("Existing turbines", "Candidate sites")),
                 probs = c(0.95, 0.8, 0.5), show.legend = TRUE) +
  geom_vline(data = wpr_means, aes(xintercept = MT_mean), color = "white") +
  geom_hline(data = wpr_means, aes(yintercept = pd_mean), color = "white") +
  # geom_hline(yintercept = mean(wpr_data$power_density, na.rm = TRUE), color = "white") +
  ...
  # guides(alpha = "none") +
  facet_wrap(vars(season, type), nrow = 1)
ggsave("powerdensity_vs_migpassage_nhol.pdf", width = 17, height = 5)

ggplot() +
  geom_bin_2d(aes(x = MT, y = power_density, fill = after_stat(ndensity)),
              data = wpr_data %>% filter(type == "Background", season == "Autumn") %>% 
                dplyr::select(MT, power_density, season), bins = 30) +
  # geom_hdr_lines(aes(x = MT, y = power_density, color = type),
  #                data = wpr_data %>% filter(type == "Candidate sites", season == "Autumn"),
  #                probs = c(0.95, 0.8, 0.5), show.legend = TRUE) +
  geom_point(aes(x = MT_mean, y = pd_mean, size = area_size), data = wpr_means_res, color = "red") +
  geom_vline(data = wpr_means, aes(xintercept = MT_mean), color = "white") +
  geom_hline(data = wpr_means, aes(yintercept = pd_mean), color = "white") +
  ...
  labs(x = "Migration passage [birds/km]",
       y = "Wind power density [W/m2]")
ggsave("powerdensity_vs_migpassage_nhol_autumn.pdf", width = 6, height = 6)
  # guides(alpha = "none")
```
