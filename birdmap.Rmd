...
nl_average_ban <- extract(ban, vect(nl_clut), raw = TRUE) %>% as.data.frame() %>% summarise(avg = mean(focal_mean, na.rm = TRUE))
nl_average_bsn <- extract(bsn, vect(nl_clut), raw = TRUE) %>% as.data.frame() %>% summarise(avg = mean(focal_mean, na.rm = TRUE))
