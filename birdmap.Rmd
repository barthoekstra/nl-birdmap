...
```
The dataset is sorted from largest night to smallest night in terms of cumulative passage, so we can select the peak nights that capture 50% of migration by selecting all nights that cumulatively have selected less and then the first night that pushes it above 50%.
...
And calculate some scan selection statistics
...
generate_stacks <- function(year_season, day_night, odim_code,
  ...
  rbcs <- lapply(files$rbcfile, readRDS)
  lapply(gr, function(x) {stack_rbcs(rbcs = rbcs, year_season = year_season, odimcode = odim_code, momenttype = "2h30m",
...
save_map <- function(x, overwrite = FALSE, cluttermap = FALSE, mode = "dark") {
  ...
  if (cluttermap) fstacks <- str_replace(fstacks, "_NA", "")
...
clutter_maps <- expand.grid(season = c("spring", "autumn"),
                            daytime = c("night"),
                            # thresh = c(25, 50, 100, 150, 200, 250, 300),  # Tested options
                            thresh = c(250),  # Most suitable
                            # s = c(1, 2, 3, 4, 5, 6, 7),  # Tested options
                            # n = c(13, 17, 21, 25, 29),  # Tested options
                            s = c(4),  # Most suitable
                            n = c(25),  # Most suitable
                            momenttype = unique(vp_mtr_cum50_cl$momenttype),
                            stringsAsFactors = FALSE)
...
We use the vertical profiles to quantify how many birds actually pass over the Netherlands seasonally. This requires downloading the necessary pvol files or VPTS' first. See the [Aloft data repository](https://aloftdata.eu/browse/?prefix=uva/) for quick access to VPTS files.
...
```{r, results='hide'}
ban_dens <- rast("data/final_maps/NL_autumn_night_thresh_250_s_4_n_25_2h30m.tif")  # Actually VIRs
...
```{r}
terra::extract(ban_dens, vect(nl_clut), raw = TRUE) %>% as.data.frame() %>% mutate(focal_mean = focal_mean / 11) %>% skim(focal_mean)
terra::extract(bsn_dens, vect(nl_clut), raw = TRUE) %>% as.data.frame() %>% mutate(focal_mean = focal_mean / 11) %>% skim(focal_mean)
terra::extract(ban_dens, vect(nhol_clut), raw = TRUE) %>% as.data.frame() %>% mutate(focal_mean = focal_mean / 11) %>% skim(focal_mean)
terra::extract(bsn_dens, vect(nhol_clut), raw = TRUE) %>% as.data.frame() %>% mutate(focal_mean = focal_mean / 11) %>% skim(focal_mean)
```

```{r}
terra::extract(ban, vect(nl_clut), raw = TRUE) %>% as.data.frame() %>% skim(focal_mean) # Autumn
terra::extract(bsn, vect(nl_clut), raw = TRUE) %>% as.data.frame() %>% skim(focal_mean) # Spring
terra::extract(ban, vect(nhol_clut), raw = TRUE) %>% as.data.frame() %>% skim(focal_mean) # Autumn
terra::extract(bsn, vect(nhol_clut), raw = TRUE) %>% as.data.frame() %>% skim(focal_mean) # Spring
```

### Preparing data package

```{r}
package_folder <- "datapackage/"
rcs <- 11  # Radar cross-section
# Final maps
nlbuf <- vect(nl_buf)
terra::mask(ban, nlbuf) %>% terra::writeRaster(glue::glue("{package_folder}NL_autumn_seasonal_passage.tif"))
terra::mask(bsn, nlbuf) %>% terra::writeRaster(glue::glue("{package_folder}NL_spring_seasonal_passage.tif"))
terra::mask(ban_dens / rcs, nlbuf) %>% terra::writeRaster(glue::glue("{package_folder}NL_autumn_bird_density.tif"))
terra::mask(bsn_dens / rcs, nlbuf) %>% terra::writeRaster(glue::glue("{package_folder}NL_spring_bird_density.tif"))
# Unsmoothed maps
...
terra::mask(raw_a, nlbuf) %>% terra::writeRaster(glue::glue("{package_folder}NL_autumn_VIR_raw.tif"))
terra::mask(raw_s, nlbuf) %>% terra::writeRaster(glue::glue("{package_folder}NL_spring_VIR_raw.tif"))

# Uncertainty areas
sf::write_sf(uncertainty, glue::glue("{package_folder}NL_uncertainty_areas.geojson"))
```
...
# Color palette from Strode et al 10.14714/CP94.1538
# Tweaked to contain an orange (high x, low y) in the inferno scale
custom_pal <- c(
  "1-1" = "#eeeeee", # low x, low y
  "2-1" = "#fedcb7",
  "3-1" = "#feba67",
  # "4-1" = "#fc9400", # high x, low y f98e09
  "4-1" = "#f98e09",
  "1-2" = "#b9ddff",
  "2-2" = "#b8b8b8",
  "3-2" = "#b89265",
  ...
  "3-3" = "#666767",
  "4-3" = "#6a3700",
  "1-4" = "#0195ff", # low x, high y
  "2-4" = "#006ab8",
  "3-4" = "#003866",
  "4-4" = "#000000" # high x, high y
)

bi_pal(pal = custom_pal, dim = 4)
...
distrib <- ggplot() +
  # stat_bin2d(aes(x = MT_disp, y = power_density_disp, fill = bi_class, alpha = after_stat(ndensity)),
  stat_bin2d(aes(x = MT_disp, y = power_density_disp, fill = bi_class),
              data = wpr_data %>% filter(type == "Background") %>%
                dplyr::select(MT_disp, power_density_disp, season, bi_class),
              hjust = 0, vjust = 0, show.legend = FALSE, geom = "raster", binwidth = c(25, 25)) +
  # geom_raster(aes(x = MT_disp, y = power_density_disp, fill = bi_class, alpha = after_stat(ndens)),
  #             data = wpr_data %>% filter(type == "Background") %>%
  #               dplyr::select(MT_disp, power_density_disp, season, bi_class) %>%
  #               distinct(),
...
```{r}
ggsave(filename = "data/final_maps_plots/powerdens_vs_migpassage_map.pdf", plot = finalplot, width = 8, height = 7.5)
ggsave(filename = "data/final_maps_plots/powerdens_vs_migpassage_distrib.pdf", plot = distrib, width = 15, height = 7.5)
```
...
vp_mtr_cum50 %>% 
  group_by(season, odim_code, year) %>% 
  summarise(nights_needed = max(top)) %>% 
  group_by(season) %>% 
  summarise(mean_nights = mean(nights_needed), 
            min_nights = min(nights_needed), 
            max_nights = max(nights_needed))
```
